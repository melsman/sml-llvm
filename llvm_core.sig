(** Core API.
    ...
    This interface provides a Standard ML API for the LLVM intermediate
    representation, the classes in the VMCore library. *)

signature LLVM_CORE = sig
...
(** [set_target_triple triple m] changes the target specifier for the module [m] to
...
(** [var_arg_function_type ret_ty param_tys] is just like
    ...
    See the method [llvm::FunctionType::get]. *)
val var_arg_function_type : lltype -> lltype list -> lltype
...
(** [vector_size ty] returns the element count of the vector type [ty].
...
(** [builder_at c ip] creates an instruction builder positioned at [ip] with
    respect to the context [c]. See the constructor for [llvm::LLVMBuilder]. *)
(*
val builder_at : llcontext -> (llbasicblock, llvalue) llpos -> llbuilder
*)

(** [builder_before c ins] creates an instruction builder positioned before the
    instruction [isn] with respect to the context [c]. See the constructor for
    [llvm::LLVMBuilder]. *)
(*
val builder_before : llcontext -> llvalue -> llbuilder
*)

(** [builder_at_end c bb] creates an instruction builder positioned at the end of
    the basic block [bb] with respect to [c]. See the constructor for
    [llvm::LLVMBuilder]. *)
...
(*
  ...
  val of_int32 : lltype -> Int32.int -> t
*)  
...
structure ExecutionEngine : sig
  ...
  (** [create m] creates a new execution engine, taking ownership of the
      ...
      interpreter. Raises [Fail msg] if an error occurrs. The execution engine
  ...
  (** [create_interpreter m] creates a new interpreter, taking ownership of the
      module [m] if successful. Raises [Fail msg] if an error occurrs. The
  ...
  (** [create_jit m optlevel] creates a new JIT (just-in-time compiler), taking
      ...
      level [optlevel]. Raises [Fail msg] if an error occurrs. The execution
...
(*  
  ...
  val free_machine_code : llvalue -> t -> unit
